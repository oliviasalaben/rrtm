...
def read_output_rrtm_funct(IMMAX, Rs, sza):
    ...
    data = np.array(pd.read_table('./rrtm_output/biomass_burning_OUTPUT_RRTM_sw_Rs{:}_sza{:}.clear'.format(Rs,sza),header=3,\
    ...
    data = np.array(pd.read_table('./rrtm_output/biomass_burning_OUTPUT_RRTM_sw_Rs{:}_sza{:}.aerosol'.format(Rs,sza),header=3,\
            names=['a','b','c','d','e','f','g'], nrows=IMMAX, delim_whitespace=True)).astype(float)
