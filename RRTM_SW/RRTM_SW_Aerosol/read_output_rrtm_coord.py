...
def read_output_rrtm_funct_coord(IMMAX, Rs, sza,lat,lon,season):
    #file = open(fname_sw_cl,'w+')	
    data = np.array(pd.read_table('./rrtm_output/NAf_OUTPUT_RRTM_sw_lat{:.2f}_lon{:.2f}_seas{:}_shrink.clear'.format(lat,lon,season),header=3,\
    ...
    data = np.array(pd.read_table('./rrtm_output/NAf_OUTPUT_RRTM_sw_lat{:.2f}_lon{:.2f}_seas{:}_shrink.aerosol'.format(lat,lon,season),header=3,\
            names=['a','b','c','d','e','f','g'], nrows=IMMAX, delim_whitespace=True)).astype(float)
