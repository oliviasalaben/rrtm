...
def read_output_rrtm_funct_coord(IMMAX, Rs, sza,lat,lon,season):
    ...
    data = np.array(pd.read_table('./rrtm_output/NAf_OUTPUT_RRTM_sw_lat{:.2f}_lon{:.2f}_seas{:}.clear'.format(lat,lon,season),header=3,\
    ...
    data = np.array(pd.read_table('./rrtm_output/NAf_OUTPUT_RRTM_sw_lat{:.2f}_lon{:.2f}_seas{:}.aerosol'.format(lat,lon,season),header=3,\
            names=['a','b','c','d','e','f','g'], nrows=IMMAX, delim_whitespace=True)).astype(float)
